...
SELECT l.nombre AS locacion, l.capacidad_disponible - SUM(pl.cantidad) AS espacio_restante
...
-- 60. Comparación entre costos de producción y ventas por mes
...
-- 77. Maquinarias con estado crítico o en mantenimiento
SELECT m.descripcion, m.marca, m.modelo, est.estado
FROM Maquinas m
JOIN Estados est ON m.id_estado = est.id_estado
WHERE est.estado IN ('Crítico', 'En Mantenimiento');

-- 78. Historial de uso de equipos en tareas
SELECT eq.descripcion, COUNT(at.id_tarea) AS veces_utilizado, MAX(t.fecha) AS ultima_vez
FROM Activos_en_tarea at
JOIN Equipos_de_trabajo eq ON at.id_activo = eq.id_activo
JOIN Tareas t ON at.id_tarea = t.id_tarea
GROUP BY eq.id_activo;
